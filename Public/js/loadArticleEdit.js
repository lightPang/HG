  ...
  $.ajax({
    ...
    success: function(data){
      ...
      for ( var i = 0 ; i<pageData.length; ++ i ){
        articleContent += "<div class='article'>" +pageData[i].content + "<button style='margin-bottom:15px;' class=\"btn btn-default\" type=\"button\" onclick=\"edit(" + pageData[i].n_id + ")\">编辑</button>"+ "<button style='margin-bottom:15px; margin-left:15px;' class=\"btn btn-default\" type=\"button\" onclick=\"del(" + pageData[i].n_id + ")\">删除</button>"+"</div>";
      } 
      $('#article-list').empty() ;
...
function edit(id){
  window.location.href="/HG/index.php/NewsEdit/edit/n_id/" + id ;  
}

function del(id){
  var flag =confirm("确认要删除这篇文章吗？");
  if( flag == true ){
  window.location.href="/HG/index.php/NewsEdit/del/n_id/" + id ;  }
}